...
const globalBucket = new (require("../util/GlobalBucket"))();
const logger = require('../../../../src/utils/logger.js');
const enableGlobalQueue = true;
            ...
            const actualCall = (cb) => {
                    ...
                    _respStream.on("data", (str) => {
                        ...
                        if(retryAfter >= 0) {
                            if(resp.headers["x-ratelimit-global"]) {
                                this.globalBlock = true;
																if (!cloudflare) {
																	globalBucket.block(retryAfter || 1)
																}
                        ...
                        if(resp.statusCode !== 429) {
                            ...
                            this._client.emit("debug", `${content}${now} ${route} ${resp.statusCode}: ${latency}ms (${this.latencyRef.latency}ms avg) | ${this.ratelimits[route].remaining}/${this.ratelimits[route].limit} left | Reset ${this.ratelimits[route].reset} (${this.ratelimits[route].reset - now}ms left)`);
														logger.increment("request", [
															'global:'+!!resp.headers["x-ratelimit-global"],
                ...
                if(Array.isArray(data)) {
                    ...
                    req.end(data);
						...
						const globalActualCall = (cb) => {
							globalBucket.queue((globalcb) => {
								const newcb = () => {
									globalcb();
									cb();
								}
                ...
                this.readyQueue.push(() => {
                    if(!this.ratelimits[route]) {
                        this.ratelimits[route] = new SequentialBucket(1, this.latencyRef);
										...
										if (enableGlobalQueue) {
                    	this.ratelimits[route].queue(globalActualCall, short);
										} else {
